...
import LeaveAbsencelessLetter from "@/letters/LeaveAbsencelessLetter.vue";
import { BankLoanRequest } from "@/models/BankLoan";
import { LeaveAbsenceLessRequest } from "@/models/LeaveAbsenceLess";
...
const { run: submitConfirmStudying } = useJob(
  (payload: LeaveAbsenceLessRequest) => {
    return LeaveAbsenceLessRepository.create(payload).pipe(
      map(({ data }) => {
...
const handleFinish = (values: LeaveAbsenceLessRequest) => {
