...
import RelssuanceTranscriptsLetter from "@/letters/RelssuanceTranscriptsLetter.vue";
import { FormService, StatusLetter } from "@/models/Letter";
import { ResolveWorkRequest } from "@/models/ResolveWork";
...
const { run: submitLetter } = useJob(
  (payload: ResolveWorkRequest) => {
    return ResolveWorkRepository.create(payload).pipe(
      map(({ data }) => {
...
const handleFinish = (values: ResolveWorkRequest) => {
