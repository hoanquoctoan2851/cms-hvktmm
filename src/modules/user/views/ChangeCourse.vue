...
import ChangeCourseLetter from "@/letters/ChangeCourseLetter.vue";
...
const { run: submitApi } = useJob(
  (payload: ChangeCourseRequest) => {
    return ChangeCourseRepository.create(payload).pipe(
      map(({ data }) => {
...
const handleFinish = (values: any) => {
  if(infoUser?.value?.balance && infoUser?.value?.balance >= MIN_PAYMENT) {
    if (user?.value?.userId) {
      const payload: ChangeCourseRequest = {
        ...values,
