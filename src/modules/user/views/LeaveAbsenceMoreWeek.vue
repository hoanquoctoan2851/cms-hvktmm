...
import LeaveAbsenceMoreLetter from "@/letters/LeaveAbsenceMoreLetter.vue";
...
const { run: submitConfirmStudying } = useJob(
  (payload: LeaveAbsenceMoreRequest) => {
    return LeaveAbsenceMoreRepository.create(payload).pipe(
      map(({ data }) => {
...
const handleFinish = (values: LeaveAbsenceMoreRequest) => {
