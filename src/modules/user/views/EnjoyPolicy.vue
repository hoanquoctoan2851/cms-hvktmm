...
import EnjoyPolicyLetter from "@/letters/EnjoyPolicyLetter.vue";
...
const { run: submitLetter } = useJob(
  (payload: EnjoyPolicyRequest) => {
    return EnjoyPolicyRepository.create(payload).pipe(
      map(({ data }) => {
...
const handleFinish = (values: EnjoyPolicyRequest) => {
