...
import ContinueStudyingLetter from "@/letters/ContinueStudyingLetter.vue";
...
const { run: submitConfirmStudying } = useJob(
  (payload: ContinueStudyingRequest) => {
    return ContinueStudyingRepository.create(payload).pipe(
      map(({ data }) => {
