...
import ResignationStudyingLetter from "@/letters/ResignationStudyingLetter.vue";
import { LeavingSchoolRequest } from "@/models/LeavingSchool";
...
const { run: submitConfirmStudying } = useJob(
  (payload: LeavingSchoolRequest) => {
    return LeavingSchoolRepository.create(payload).pipe(
      map(({ data }) => {
...
const handleFinish = (values: LeavingSchoolRequest) => {
