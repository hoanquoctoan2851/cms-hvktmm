...
import RenewCardLetter from "@/letters/RenewCardLetter.vue";
...
const { run: submitLetter } = useJob(
  (payload: RenewStudentCardRequest) => {
    return RenewStudentCardRepository.create(payload).pipe(
      map(({ data }) => {
...
const handleFinish = (values: RenewStudentCardRequest) => {
