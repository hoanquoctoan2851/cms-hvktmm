...
import PaymentCollectiveLetter from "@/letters/PaymentCollectiveLetter.vue";
import { FormService, StatusLetter } from "@/models/Letter";
import { PaymentGraduationClassRequest } from "@/models/PaymentGraduationClass";
...
const { run: submitConfirmStudying } = useJob(
  (payload: PaymentGraduationClassRequest) => {
    return PaymentGraduationClassRepository.create(payload).pipe(
      map(({ data }) => {
